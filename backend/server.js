...
// Route to handle form submission and show success page
app.post('/book-appointment', async (req, res) => {
  const {
    patient_name,
    age,
  ...
  try {
    ...
    if (patientResult.rows.length === 0) {
      // Insert new patient if not exists
      const insertPatientQuery = `
        INSERT INTO patients (patient_id, patient_name, age, weight, pincode, mail, disease, severity)
        VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7)
        RETURNING patient_id;
      `;
      const insertPatientResult = await client.query(insertPatientQuery, [patient_name, age, weight, pincode, mail, disease, severity]);
      patient_id = insertPatientResult.rows[0].patient_id;
    } else {
      // Use existing patient_id
      patient_id = patientResult.rows[0].patient_id;
    ...
    // Insert the appointment into the database
    const insertAppointmentQuery = `
      INSERT INTO appointments (doctor_id, doctor_name, specialization, appointment_date, patient_id, patient_name, queue_number, disease, severity, severity_order, age, weight, mail, pincode)
    ...
    client.release();
    res.redirect('/appointment-success'); // Redirect to the success page after booking
  } catch (err) {
    console.error('Error booking appointment:', err);
    res.status(500).send('Internal Server Error');
  }
...
// Route to show the booking success page
